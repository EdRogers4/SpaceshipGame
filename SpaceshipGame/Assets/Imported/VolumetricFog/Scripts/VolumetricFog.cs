...
namespace VolumetricFogAndMist {
    ...
    public partial class VolumetricFog : MonoBehaviour {
        ...
        const string VFM_BUILD_HINT = "VFMBuildHint109b1";
...
#region Settings area
        ...
        void UpdateMaterialHeights(Camera mainCamera) {
            ...
            fogMat.SetVector(ShaderParams.FogData, new Vector4(currentFogAltitude, _height, 1.0f / (_density * temporaryProperties.density), scale));
        ...
        void UpdateMaterialPropertiesNow(Camera mainCamera) {
            ...
            if (_fogOfWarEnabled) {
                ...
                if (_useXYPlane) {
                    fogMat.SetVector(ShaderParams.FogOfWarCenterAdjusted, new Vector4(ca.x / _fogOfWarSize.x, ca.y / (_fogOfWarSize.y + 0.0001f), 1f));
                } else {
                    fogMat.SetVector(ShaderParams.FogOfWarCenterAdjusted, new Vector4(ca.x / _fogOfWarSize.x, 1f, ca.z / (_fogOfWarSize.z + 0.0001f)));
                }
            }
...
#if UNITY_EDITOR
        void OnDrawGizmos() {
            if (_fogAreaShowGizmos && _fogAreaRadius > 0) {
                if (_fogAreaTopology == FOG_AREA_TOPOLOGY.Box) {
